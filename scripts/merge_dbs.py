    ...
    try:
        ...
        for table in tables_to_copy:
            if table not in main_tables:
                print(
                    f"[!] Target table '{table}' does not exist in portfolio.db yet. Skipping. Run app first."
                )
                continue
            ...
            cursor.execute(
                f"INSERT OR IGNORE INTO main.{table} ({cols_str}) SELECT {cols_str} FROM cmi.{table}"
            )
