...
class RoombaTestCase(unittest.TestCase):
    ...
    def test_playlist_import_idempotency(self):
        ...
        # Try to import again
        resp = self.client.post("/music/roomba/api/roomba/import", data=json.dumps({
            "playlist_id": "3n3Ppam7vgaVa1iaRUIOKE",
            "playlist_name": "Test Playlist"
    ...
    def test_transitions_ownership(self):
        ...
        pi = PlaylistImport(user_id=self.user_b.id, source_playlist_id="3n3Ppam7vgaVa1iaRUIOKE", status="complete", is_active=True)
        db.session.add(pi)
        db.session.flush()
    ...
    def test_track_features_ownership(self):
        ...
        self.login_user("usera@example.com")
        resp = self.client.put(f"/music/roomba/api/roomba/track/{t1.id}/features", data=json.dumps({
            "camelot_key": "9A"
        }), content_type="application/json")
    ...
    def test_camelot_key_validation(self):
        ...
        pi = PlaylistImport(user_id=self.user_a.id, source_playlist_id="3n3Ppam7vgaVa1iaRUIOKE", status="complete", is_active=True)
        db.session.add(pi)
        db.session.flush()
    ...
    def test_n_plus_one_queries(self):
        ...
        for idx in range(10):
            t = TrackIdentity(spotify_track_id=f"trackspotifyid{idx}", title=f"Track {idx}", artist="Artist")
            db.session.add(t)
            db.session.flush()
            ...
            # Seed annotations
            anno = TrackTempoAnnotation(track_id=t.id, canonical_bpm=120.0, confidence="verified", source="manual")
            db.session.add(anno)
    ...
    def test_transition_candidate_compute_on_demand(self):
        ...
        for idx in range(3):
            ...
            anno = TrackTempoAnnotation(track_id=t.id, canonical_bpm=120.0, camelot_key="8A", confidence="verified", source="manual")
            db.session.add(anno)
