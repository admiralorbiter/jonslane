...
from portfolio.models import AnchorSchedule, User, seed_database
from portfolio.utils.srs import (
    get_due_anchors,
    seed_schedule_from_history,
    update_schedule_after_attempt,
)
...
class SRSTestCase(unittest.TestCase):
    ...
    def test_metrical_match_streak_capping(self):
        ...
        update_schedule_after_attempt(
            user_id, anchor_bpm, "Metrical Match", base_time + timedelta(hours=19)
        )
        schedule = AnchorSchedule.query.filter_by(user_id=user_id, anchor_bpm=anchor_bpm).first()
        ...
        update_schedule_after_attempt(
            user_id, anchor_bpm, "Metrical Match", base_time + timedelta(hours=38)
        )
        schedule = AnchorSchedule.query.filter_by(user_id=user_id, anchor_bpm=anchor_bpm).first()
        ...
        update_schedule_after_attempt(
            user_id, anchor_bpm, "DJ-Ready", base_time + timedelta(hours=57)
        )
        schedule = AnchorSchedule.query.filter_by(user_id=user_id, anchor_bpm=anchor_bpm).first()
