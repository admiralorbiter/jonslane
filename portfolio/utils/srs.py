...
def get_due_anchors(user_id: int) -> list[dict]:
    ...
    schedules = AnchorSchedule.query.filter(
        ...
        (AnchorSchedule.next_review_at == None) | (AnchorSchedule.next_review_at <= now)
    ).all()
...
def update_schedule_after_attempt(user_id: int, anchor_bpm: int, rating: str, attempt_time: datetime = None) -> None:
    """
