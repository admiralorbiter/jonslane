...
from portfolio.routes.roomba_scoring import score_transition, bucket_candidates
...
def get_resolved_features(track_id: int) -> dict:
    ...
    librosa_feat = TrackFeatureAnnotation.query.filter_by(track_id=track_id, source="librosa").first()
...
def get_resolved_features_cached(track_id: int, tempos: dict, manual_feats: dict, librosa_feats: dict) -> dict:
        ...
        try:
            ...
            for track_id, artist, title in track_infos:
                try:
                    ...
                    app.logger.error("Error in background track analysis loop for track %s: %s", track_id, str(e))
                    try:
                        db.session.rollback()
                    except Exception:
                        pass
...
def api_import():
    ...
    recent = (PlaylistImport.query
              .filter_by(user_id=user_id, source_playlist_id=playlist_id)
              .order_by(PlaylistImport.imported_at.desc())
              .first())
    if recent and (datetime.now(timezone.utc) - recent.imported_at.replace(tzinfo=timezone.utc)) < timedelta(minutes=5):
        return jsonify({
    ...
    playlist_import = PlaylistImport.query.filter_by(user_id=user_id, source_playlist_id=playlist_id).first()
    if playlist_import:
    ...
    for idx, track_data in enumerate(tracks):
        ...
        if not identity:
            ...
            art_url = images[1].get("url") if len(images) > 1 else (images[0].get("url") if images else None)
        ...
        pt = PlaylistTrack(
            playlist_id=playlist_import.id,
    ...
    thread = threading.Thread(
        target=playlist_import_worker,
        args=(app, playlist_import.id, track_infos)
    )
    thread.daemon = True
    thread.start()
...
@roomba_bp.route("/api/roomba/playlist/<int:id>")
def api_playlist_detail(id):
    """Fetch detail for a specific playlist import, including all tracks."""
    user_id = session.get("user_id")
    playlist = PlaylistImport.query.filter_by(id=id, user_id=user_id, is_active=True).first()
    if not playlist:
        return jsonify({"error": "Playlist import not found"}), 404
        
    # Eager load tracks to avoid N+1
    playlist_tracks = (PlaylistTrack.query
                       .filter_by(playlist_id=id)
                       .order_by(PlaylistTrack.position)
                       .all())
    track_ids = [pt.track_id for pt in playlist_tracks]
    
    identities = {t.id: t for t in TrackIdentity.query.filter(TrackIdentity.id.in_(track_ids)).all()}
    
    # Batch load annotations to prevent N+1
    tempos = {t.track_id: t for t in TrackTempoAnnotation.query.filter(TrackTempoAnnotation.track_id.in_(track_ids)).all()}
...
@roomba_bp.route("/api/roomba/track/<int:id>/features", methods=["PUT"])
def api_update_features(id):
    ...
    exists = (db.session.query(PlaylistTrack)
              .join(PlaylistImport)
              .filter(PlaylistImport.user_id == user_id, PlaylistTrack.track_id == id)
              .first())
    if not exists:
    ...
    tempo = TrackTempoAnnotation.query.filter_by(track_id=id).first()
    if manual_bpm is not None:
        if tempo:
    ...
    tfa = TrackFeatureAnnotation.query.filter_by(track_id=id, source="manual").first()
    if not tfa:
        tfa = TrackFeatureAnnotation(
            track_id=id,
            source="manual",
            confidence=1.0
        )
    ...
    TransitionCandidate.query.filter(
        (TransitionCandidate.from_track_id == id) | (TransitionCandidate.to_track_id == id)
    ).update({TransitionCandidate.status: "stale", TransitionCandidate.stale_reason: "Track features manually updated"})
    
    db.session.commit()
    
    return jsonify({
        "success": True,
        "features": get_resolved_features(id)
    })


@roomba_bp.route("/api/roomba/transitions/<int:playlist_id>/<int:track_id>")
def api_transitions(playlist_id, track_id):
    ...
    playlist = PlaylistImport.query.filter_by(id=playlist_id, user_id=user_id, is_active=True).first()
    if not playlist:
    ...
    tempos = {t.track_id: t for t in TrackTempoAnnotation.query.filter(TrackTempoAnnotation.track_id.in_(all_track_ids)).all()}
    tfas = TrackFeatureAnnotation.query.filter(TrackFeatureAnnotation.track_id.in_(all_track_ids)).all()
    ...
    existing_tcs = {tc.to_track_id: tc for tc in TransitionCandidate.query.filter_by(
    ...
    for candidate_id in candidate_ids:
        ...
        if needs_compute:
            features_b = get_resolved_features_cached(candidate_id, tempos, manual_feats, librosa_feats)
    ...
    candidates = (TransitionCandidate.query
                  .filter_by(playlist_id=playlist_id, from_track_id=track_id, status="computed")
    ...
    for c in candidates:
        ...
        bucketed_candidates.append({
            ...
            "score_data": {
                ...
                "tempo_label": c.explanation_json.get("tempo_label", "unknown") if c.explanation_json else "unknown",
                "harmonic_score": c.harmonic_score,
                "harmonic_label": c.explanation_json.get("harmonic_label", "unknown") if c.explanation_json else "unknown",
                "energy_score": c.energy_score,
                "energy_direction": c.explanation_json.get("energy_direction", "unknown") if c.explanation_json else "unknown",
